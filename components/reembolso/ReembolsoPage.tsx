...
export default function ReembolsoPage() {
  ...
  const { despesas, isLoading, processarReembolso, estornarReembolso } = useDespesas(
    undefined,
    currentUser?.perfil
  );
  ...
  const [filterStatus, setFilterStatus] = useState<"todos" | "pendente" | "processado">("pendente");
  ...
  const despesasReembolso = useMemo(() => {
    return despesas
      ...
      .filter((d) => {
        if (filterStatus === "pendente") return !d.reembolso_processado;
        if (filterStatus === "processado") return d.reembolso_processado;
        return true;
      })
      .filter((d) => {
      ...
      {despesasReembolso.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {despesasReembolso.map((d) => {
            ...
            const statusKey = d.reembolso_processado ? "processado" : "pendente";
                  ...
                  <div className="px-4 pb-4 border-t border-border pt-4 space-y-4">
                    ...
                    {d.reembolso_processado ? (
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-success/10 border border-success/20 text-success text-sm">
                        ...
                        <span className="font-medium">Reembolso processado pelo financeiro</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2 p-3 rounded-lg bg-warning/10 border border-warning/20 text-warning text-sm">
                        <Clock className="w-4 h-4 shrink-0" />
                        <span className="font-medium">Aguardando processamento do reembolso</span>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {!d.reembolso_processado ? (
                        ...
                        >
                          ...
                          {isLoading ? "Revertendo..." : "Reverter para Pendente"}
                        </button>
                      )}
                    </div>
                  </div>
